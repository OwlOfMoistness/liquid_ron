...
contract MockRonStaking is IRoninValidator {
	...
	uint256 constant public _3_DAYS = 3 days;
	// 12 * 10**16 / 365 days
	uint256 constant public RATE_PER_SECOND = 3805175038;
	...
	function undelegate(address _consensusAddr, uint256 _amount) public {
		...
		require(lastStakeAction[_consensusAddr][msg.sender] + _3_DAYS < block.timestamp, "MockRonStaking: must wait 3 days after last stake action");
	...
	function redelegate(address _consensusAddrSrc, address _consensusAddrDst, uint256 amount) external {
		...
		require(lastStakeAction[_consensusAddrSrc][msg.sender] + _3_DAYS < block.timestamp, "MockRonStaking: must wait 3 days after last stake action");
	...
	function delegateRewards(address[] calldata _consensusAddrs, address _consensusAddrDst) external returns (uint256 rewards) {
	...
	function getManyStakingAmounts(address[] calldata _consensusAddrs,address[] calldata userList) external view returns (uint256[] memory) {
